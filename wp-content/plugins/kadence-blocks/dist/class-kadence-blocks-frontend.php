...
class Kadence_Blocks_Frontend {
	...
	public function render_advanced_btn_css( $attributes, $content ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_inline_css', true, 'advancedbtn', $unique_id ) ) {
				...
				$this->should_render_inline_stylesheet( 'kadence-blocks-btn' );
				$css = $this->blocks_advanced_btn_array( $attributes, $unique_id );
				if ( ! empty( $css ) ) {
	...
	public function render_tabs_css( $attributes, $content ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_inline_css', true, 'tabs', $unique_id ) ) {
				// If filter didn't run in header (which would have enqueued the specific css id ) then filter attributes for easier dynamic css.
				$this->should_render_inline_stylesheet( 'kadence-blocks-tabs' );
				$attributes = apply_filters( 'kadence_blocks_tabs_render_block_attributes', $attributes );
				$css = $this->blocks_tabs_array( $attributes, $unique_id );
				if ( ! empty( $css ) ) {
					if ( $this->should_render_inline( 'tabs', $unique_id ) ) {
	...
	public function render_google_maps_css_head( $attributes ) {
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_head_css', true, 'google_maps', $attributes ) ) {
				// Filter attributes for easier dynamic css.
	...
	public function render_showmore_css_head( $attributes ) {
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_head_css', true, 'show-more', $attributes ) ) {
				// Filter attributes for easier dynamic css.
	...
	public function render_showmore_css( $attributes, $content ) {
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			$style_id = 'kb-show-more' . esc_attr( $unique_id );
			if ( $this->it_is_not_amp() ) {
				wp_enqueue_script( 'kadence-show-more' );
			}
	...
	public function render_lottie_css_head( $attributes ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			$unique_id = $attributes['uniqueID'];
			$style_id = 'kb-lottie' . esc_attr( $unique_id );
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_head_css', true, 'lottie', $attributes ) ) {
				// If filter didn't run in header (which would have enqueued the specific css id ) then filter attributes for easier dynamic css.
				$attributes = apply_filters( 'kadence_blocks_lottie_render_block_attributes', $attributes );
				$css = $this->blocks_lottie_array( $attributes, $unique_id );
				if ( ! empty( $css ) ) {
	...
	public function render_image_css_head( $attributes ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			$unique_id = $attributes['uniqueID'];
			$style_id = 'kb-image' . esc_attr( $unique_id );
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_head_css', true, 'image', $attributes ) ) {
				// If filter didn't run in header (which would have enqueued the specific css id ) then filter attributes for easier dynamic css.
				$attributes = apply_filters( 'kadence_blocks_image_render_block_attributes', $attributes );
				$css = $this->blocks_image_array( $attributes, $unique_id );
				if ( ! empty( $css ) ) {
			...
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_inline_css', true, 'testimonials', $unique_id ) ) {
				...
				if ( isset( $attributes['layout'] ) && 'carousel' === $attributes['layout'] ) {
					if ( $this->it_is_not_amp() ) {
						...
						$this->should_render_inline_stylesheet( 'kadence-blocks-tiny-slider' );
						wp_enqueue_script( 'kadence-blocks-tiny-slider-init' );
					}
				}
				$this->should_render_inline_stylesheet( 'kadence-blocks-testimonials' );
				$css = $this->blocks_testimonials_array( $attributes, $unique_id );
				if ( ! empty( $css ) ) {
	...
	public function render_advancedgallery_css( $attributes, $content ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'kadence_blocks_render_inline_css', true, 'advancedgallery', $unique_id ) ) {
				...
				$this->should_render_inline_stylesheet( 'kadence-blocks-gallery' );
				$css = $this->blocks_advancedgallery_array( $attributes, $unique_id );
				if ( ! empty( $css ) ) {
	...
	public function register_scripts() {
		...
		wp_register_script( 'kadence-show-more', KADENCE_BLOCKS_URL . 'dist/assets/js/kb-show-more.min.js', array(), KADENCE_BLOCKS_VERSION, true );
			...
			foreach ( $blocks as $indexkey => $block ) {
				...
				if ( ! is_object( $block ) && is_array( $block ) && isset( $block['blockName'] ) ) {
					...
					if ( 'kadence/show-more' === $block['blockName'] ) {
						if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
							$blockattr = $block['attrs'];
							$this->render_showmore_scripts( $blockattr );
	...
	public function blocks_show_more_array( $attr, $unique_id  ) {
		...
		$key_positions = array( 'top', 'right', 'bottom', 'left' );
		...
		if ( isset( $attr['heightTablet'] ) && ! empty( $attr['heightTablet'] ) ) {
			$css->start_media_query( $media_query['tablet'] );
			$css->set_selector( '.kb-block-show-more-container' . $unique_id . ' > .wp-block-kadence-column' );
			$css->add_property( 'max-height', $attr['heightTablet'] .  ( isset( $attr['heightType'] ) ? $attr['heightType'] : 'px' ) );
			$css->stop_media_query();
		}
	...
	public function blocks_icon_array( $attr, $unique_id ) {
		...
		if ( ! empty( $attr['verticalAlignment'] ) ) {
			if ( ! isset( $attr['textAlignment'] ) || ( isset( $attr['textAlignment'] ) && 'center' === $attr['textAlignment'] ) ) {
				$css .= '.wp-block-kadence-icon.kt-svg-icons' . $unique_id . '[class*="kb-icon-valign-"] {';
				$css .= 'justify-content:center';
				$css .= '}';
			}
			if ( isset( $attr['textAlignment'] ) && 'right' === $attr['textAlignment'] ) {
				$css .= '.wp-block-kadence-icon.kt-svg-icons' . $unique_id . '[class*="kb-icon-valign-"] {';
				$css .= 'justify-content:flex-end';
				$css .= '}';
			}
			if ( isset( $attr['textAlignment'] ) && 'left' === $attr['textAlignment'] ) {
				$css .= '.wp-block-kadence-icon.kt-svg-icons' . $unique_id . '[class*="kb-icon-valign-"] {';
		...
		if ( isset( $attr['dividerStyle'] ) && ! empty( $attr['dividerStyle'] ) && 'stripe' === $attr['dividerStyle'] ) {
			...
			if ( ( isset( $attr['tabletDividerHeight'] ) && ! empty( $attr['tabletDividerHeight'] ) ) || ( isset( $attr['tabletDividerWidth'] ) && ! empty( $attr['tabletDividerWidth'] ) ) ) {
				...
				if ( isset( $attr['tabletDividerHeight'] ) && ! empty( $attr['tabletDividerHeight'] ) ) {
					$css->add_property( 'height', $attr['tabletDividerHeight'] . 'px !important' );
				}
			...
			if ( ( isset( $attr['mobileDividerHeight'] ) && ! empty( $attr['mobileDividerHeight'] ) ) || ( isset( $attr['mobileDividerWidth'] ) && ! empty( $attr['mobileDividerWidth'] ) ) ) {
				...
				if ( isset( $attr['mobileDividerHeight'] ) && ! empty( $attr['mobileDividerHeight'] ) ) {
					$css->add_property( 'height', $attr['mobileDividerHeight'] . 'px !important' );
				}
			...
			if ( ( isset( $attr['tabletDividerHeight'] ) && ! empty( $attr['tabletDividerHeight'] ) ) || ( isset( $attr['tabletDividerWidth'] ) && ! empty( $attr['tabletDividerWidth'] ) ) ) {
				...
				$css->set_selector( '.wp-block-kadence-spacer.kt-block-spacer-' . $unique_id . ' .kt-divider' );
				if ( isset( $attr['tabletDividerHeight'] ) && ! empty( $attr['tabletDividerHeight'] ) ) {
					$css->add_property( 'border-top-width', $attr['tabletDividerHeight'] . 'px !important' );
				}
				if ( isset( $attr['tabletDividerWidth'] ) && ! empty( $attr['tabletDividerWidth'] ) ) {
					$css->add_property( 'width', $attr['tabletDividerWidth'] . $divider_width_units . '!important' );
				}
			...
			if ( ( isset( $attr['mobileDividerHeight'] ) && ! empty( $attr['mobileDividerHeight'] ) ) || ( isset( $attr['mobileDividerWidth'] ) && ! empty( $attr['mobileDividerWidth'] ) ) ) {
				...
				$css->set_selector( '.wp-block-kadence-spacer.kt-block-spacer-' . $unique_id . ' .kt-divider' );
				if ( isset( $attr['mobileDividerHeight'] ) && ! empty( $attr['mobileDividerHeight'] ) ) {
					$css->add_property( 'border-top-width', $attr['mobileDividerHeight'] . 'px !important' );
				}
				if ( isset( $attr['mobileDividerWidth'] ) && ! empty( $attr['mobileDividerWidth'] ) ) {
					$css->add_property( 'width', $attr['mobileDividerWidth'] . $divider_width_units . '!important' );
				}
	...
	public function blocks_tabs_array( $attr, $unique_id ) {
		...
		if ( isset( $attr['contentBorder'] ) || isset( $attr['innerPadding'] ) || isset( $attr['minHeight'] ) || isset( $attr['contentBorderColor'] ) || isset( $attr['contentBgColor'] ) ) {
			...
			if ( isset( $attr['contentBorderRadius'] ) && ! empty( $attr['contentBorderRadius'] ) && is_array( $attr['contentBorderRadius'] ) ) {
	...
	public function blocks_iconlist_array( $attr, $unique_id ) {
		...
		if ( isset( $attr['listGap'] ) && is_numeric( $attr['listGap'] ) ) {
			...
			$css .= '@media (max-width: 1024px) {';
				...
				$css .= '}';
				if ( ( ! empty( $attr['columns'] ) && 1 !== $attr['columns'] ) || ( ! empty( $attr['tabletColumns'] ) && 1 !== $attr['tabletColumns'] ) ) {
			...
			$css .= '@media (max-width: 767px) {';
				...
				$css .= '}';
				if ( ( ! empty( $attr['columns'] ) && 1 !== $attr['columns'] ) || ( ! empty( $attr['tabletColumns'] ) && 1 !== $attr['tabletColumns'] ) || ( ! empty( $attr['mobileColumns'] ) && 1 !== $attr['mobileColumns'] ) ) {
	 ...
	 * @param array $attr the blocks attr.
	 */
	public function render_showmore_scripts( $attr ) {
		$this->enqueue_script( 'kadence-show-more' );
	}
	/**
	...
	public function blocks_image_array( $attr, $unique_id ) {
		...
		if ( $align === 'right' || $align === 'left' ) {
			...
			$css->set_selector( '.wp-block-kadence-image.kb-image' . $unique_id . ' > figure.align' . $align );
		}
		// Margins
		foreach(['Desktop', 'Tablet', 'Mobile'] as $breakpoint) {
			if ( $breakpoint !== 'Desktop' ) {
	...
	public function blocks_advanced_heading_array( $attr, $unique_id ) {
		...
		if ( isset( $attr['size'] ) || isset( $attr['lineHeight'] ) || isset( $attr['typography'] ) || isset( $attr['fontWeight'] ) || isset( $attr['fontStyle'] ) || isset( $attr['textTransform'] ) || isset( $attr['letterSpacing'] ) || isset( $attr['color'] ) || isset( $attr['topMargin'] ) || isset( $attr['rightMargin'] ) || isset( $attr['bottomMargin'] ) || isset( $attr['leftMargin'] ) || isset( $attr['textShadow'] ) || isset( $attr['align'] ) || isset( $attr['padding'] ) ) {
			...
			if ( isset( $attr['color'] ) && ! empty( $attr['color'] ) ) {
				...
				} else if ( strpos( $attr['color'], 'palette' ) === 0 ) {
					$css->add_property( 'color', $css->render_color( $attr['color'] ) );
				} else if ( isset( $attr['colorClass'] ) && empty( $attr['colorClass'] ) || ! isset( $attr['colorClass'] ) ) {
					$css->add_property( 'color', $css->render_color( $attr['color'] ) );
				}
			}
			if ( isset( $attr['background'] ) && ! empty( $attr['background'] ) ) {
				if ( class_exists( 'Kadence\Theme' ) ) {
					...
					$css->add_property( 'background-color', $css->render_color( $attr['background'] ) );
				} else if ( isset( $attr['backgroundColorClass'] ) && empty( $attr['backgroundColorClass'] ) || ! isset( $attr['backgroundColorClass'] ) ) {
					$css->add_property( 'background-color', $css->render_color( $attr['background'] ) );
				}
			}
	...
	public function column_layout_css( $attr, $unique_id ) {
		...
		if ( ! empty( $attr['maxWidth'][1] ) ) {
			$css->set_selector( '.kadence-column' . $unique_id . ', .wp-block-kadence-column.kb-section-md-dir-vertical:not(.kb-section-md-dir-horizontal):not(.kb-section-md-dir-specificity)>.kt-inside-inner-col>.kadence-column' . $unique_id );
			...
			$css->set_selector( '.wp-block-kadence-column.kb-section-md-dir-horizontal>.kt-inside-inner-col>.kadence-column' . $unique_id . ', .wp-block-kadence-column.kb-section-dir-horizontal:not(.kb-section-md-dir-vertical) >.kt-inside-inner-col>.kadence-column' . $unique_id );
			$css->add_property( 'flex', '0 1 ' . $attr['maxWidth'][1] . ( isset( $attr['maxWidthUnit'] ) ? $attr['maxWidthUnit'] : 'px' ) );
			$css->add_property( 'max-width', 'unset' );
			$css->add_property( 'margin-left', 'unset' );
			$css->add_property( 'margin-right', 'unset' );
			$css->set_selector( '.wp-block-kadence-column.kb-section-md-dir-vertical>.kt-inside-inner-col>.kadence-column' . $unique_id );
			$css->add_property( 'max-width', $attr['maxWidth'][1] . ( isset( $attr['maxWidthUnit'] ) ? $attr['maxWidthUnit'] : 'px' ) );
			$css->add_property( 'margin-left', 'auto' );
			$css->add_property( 'margin-right', 'auto' );
