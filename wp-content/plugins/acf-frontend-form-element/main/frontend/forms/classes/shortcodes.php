		...
		foreach( $form['record']['fields'] as $type => $fields ){
			foreach( $fields as $key => $field ){
				...
				if( ! $input && isset( $field['default_value'] ) && is_string( $field['default_value'] ) ){
					...
					if( $dynamic_value && $dynamic_value != $field['default_value'] ) $input = $dynamic_value;
				}
		...
		switch( $field['type'] ){
			...
			case 'post_object':
				if( is_array( $value ) && $value ){
					$returns = array();
					foreach( $value as $single_value ){
						if( $field['return_format'] == 'id' || is_string( $single_value ) ) $single_value = get_post( $single_value );
