	...
	class Submit_Form {
		...
		public function check_inline_field(){
			...
			if( ! empty( $_POST['acff'] ) ){
				...
				foreach( $_POST['acff'] as $source => $fields ){
					...
					foreach( $fields as $key => $value ){
						...
						if( $valid ){
							...
							$json['updates'] .= fea_instance()->dynamic_values->display_field( $object );
						}
					}
				}
			}
		...
		public function verify_email_address(){
			...
			$submission = $fea->submissions_handler->get_submission( $request['submit_id' ] );
			...
			if( isset( $record['emails_to_verify'][$address] ) ){
				...
				$fea->submissions_handler->update_submission( $request['submit_id' ], $args );		
		...
		public function __construct(){
			...
			add_action( 'wp_ajax_frontend_admin/update_inline_field', array( $this, 'check_inline_field' ) );
			add_action( 'wp_ajax_nopriv_frontend_admin/update_inline_field', array( $this, 'check_inline_field' ) );
