	...
	class Display_Form {
		...
		public function form_set_data( $form = array() ) {
			...
			$data = wp_parse_args( $form['hidden_fields'], array(
				...
				'status'    => '',
				'message'    => '',
				'form'		=> fea_encrypt(json_encode( $form ))
			) );
		...
		public function form_render_data( $form = array() ) {
			...
			$error_msg = '';
			if( $form['error_message'] ) $error_msg = $form['error_message'];
			?>
		...
		public function render_inline_edit( $field ){ ?>
			<i class="fea-inline-edit action dashicons dashicons-edit"></i>
			<i class="fea-inline-cancel action dashicons dashicons-no"></i>
			<form class="fea-inline-form">
			<?php echo $this->render_field_wrap( $field ); ?>
			<button type="button" class="fea-inline-save button"><?php echo __( 'Save', FEA_NS ) ?></button>
			</form>
		<?php }
		...
		public function render_field_wrap( $field, $element = 'div', $instruction = 'label' ) {
			...
			if( empty( $field['no_wrap'] ) ){
				...
				if( $field['type'] == 'wysiwyg' && feadmin_edit_mode() ){
					$field['media_buttons'] = 0;
				}
				...
				if( $current_fields ){
					foreach( $current_fields as $index => $field_data ) {	
						...
						if( isset( $field_data['type'] ) ){
							...
							$exclude_in_approval = [ 'submit_button', 'save_progress' ];
		...
		public function render_fields( $current_fields = array(), $form = array(), $defaults = false, $hooked = false ){
			if( empty( $form ) ){				
				...
				foreach( $form['fields_to_display'] as $chosen_fields ){
					...
					if( is_numeric( $chosen_fields ) ){
						if( get_post_type( $chosen_fields ) == 'acf-field-group' ){
							$type_of_choice = [ 'group', $chosen_fields ];
						}
						if( get_post_type( $chosen_fields ) == 'admin_form' ){
							$type_of_choice = [ 'form', $chosen_fields ];
						}
					}
			 ...
			 */
			if( empty( $form['nested_form'] ) ) $fields = apply_filters( FEA_PREFIX.'/pre_render_fields', $fields, $form );
				...
				foreach( $fields as $field ) {
					...
					if( isset( $field['render_content'] ) ){
						...
						if( $field['key'] == '_validate_email' ){
							...
							if( ! $data_type || in_array( $data_type, [ 'general', 'mailchimp' ] ) ){
								$data_type = $cf_save;
		...
		public function render_form( $form, $preview = false ) {
			...
			if( isset( $_GET['submit_id'] ) ){
				if( isset( $_GET['email_address'] ) ){
					$address = wp_kses( $_GET['email_address'], 'strip' );
			...
			if( ! empty( $form['show_in_modal'] ) && empty( $form['page_builder'] ) ){ 
				...
				acf_hidden_input( array( 'class' => 'form-data', 'name' => 'form_'.$form['id'], 'value' => fea_encrypt( json_encode( $form ) ) ) );
				return;
			}
		...
		public function ajax_get_submissions() {
			...
			$form = $this->get_form( wp_kses( $_REQUEST['form_id'], 'strip' ) );
		...
		public function change_form() {
			...
			$form = json_decode( fea_decrypt( $_REQUEST['form_data'] ), true );
			if( !$form ) {
				wp_send_json_error();
			}
		...
		function render_field_display( $field ){
			$permissions = false;
			$show_form = apply_filters( FEA_PREFIX.'/show_form', $this->get_form( $field['parent'] ) );

			if( ! empty( $show_form['display'] ) ){
				$permissions = true;
			}
			...
			if( $permissions ){
				if( ! is_admin() ){
					global $fea_scripts_loaded;
					if( empty( $form['scripts_loaded'] ) && empty( $fea_scripts_loaded ) ){
						fea_instance()->wp_hooks->enqueue_scripts( 'frontend_admin_form' );
						fea_instance()->wp_hooks->acfdata( true );
						$fea_scripts_loaded = 1;
					}
				}
				echo '<div class="fea-display-field">';
				if( isset( $field['with_edit'] ) ){					
					echo $this->render_inline_edit( $field );
				}
				echo '<div class="fea-display-value">';
			}
		...
		public function shortcode( $atts ) {
			if ( isset( $atts['field'] ) ) {	
				...
				if( $field ){	
					...
					ob_start();
					$this->render_field_display( $field, $source );
					$output = ob_get_clean();				
					return $output;		
				}		
			}
