...
class Frontend_Forms_UI {
	...
	function save_post( $form_id, $post ) {
		...
		if( $post->post_type !== 'admin_form' && $post->post_type !== 'admin_template' ) {
			...
			if( !empty( $_POST['_acf_delete_fields'] ) ) {
				...
				// clean
				$ids = explode('|', sanitize_text_field( $_POST['_acf_delete_fields'] ) );
			...
			if( ! empty( $_POST['form'] ) ){
				...
				$form['ID'] = $form_id;
				$form['title'] = sanitize_text_field( $_POST['post_title'] );
				$form['status'] = sanitize_text_field( $_POST['post_status'] );
				$form['slug'] = sanitize_title( $_POST['post_name'] );

				if( ! empty( $_POST['post_password'] ) ){
					$form['password'] = $_POST['post_password'];
				}
				$this->update_form_post( $form );
			}				
	}
	...
	function display_shortcode() {
		...
		if ( 'admin_form' == $type || 'admin_template' == $type ){
			...
			foreach( array( 'form', 'template' ) as $content ){	
				...
				echo sprintf( '%s: <code>[frontend-admin form="%d"]</code>', __( 'Form Shortcode', FEA_NS ), $form_id );
				
				//Save icon location
				$icon_path = '<span class="dashicons dashicons-admin-page"></span>';
			
				echo sprintf( '<button type="button" class="copy-shortcode" data-prefix="frontend_admin form" data-value="%1$s">%2$s %3$s</button>',
					$form_id,
					$icon_path,
					__( 'Copy Code', FEA_NS )
				);
	...
	function get_dynamic_options(){
		$dynamic_data = array(
			__( 'Form', FEA_NS ) => array(
				'all_fields' => __( 'All Fields', FEA_NS ),
				'page_url' => __( 'Current Page URL', FEA_NS ),
				'form_name' => __( 'Form Name', FEA_NS ),
				'date' => __( 'Current Date', FEA_NS ),
				'time' => __( 'Current Time', FEA_NS ),
			...
			__( 'Post', FEA_NS ) => array(			
				...
				'post:field_name' => __( 'Custom Field', FEA_NS ),
			),
			__( 'User', FEA_NS ) => array(
				...
				'user:field_name' => __( 'Custom Field', FEA_NS ),
			),
		);		


		echo '<div class="dynamic-values">';
		acf_render_field( array(
			'field_label_hide'	=> true,
			'instructions'		=> '',
			'type'			=> 'select',
			'name'			=> 'dynamic_data_options',
			'choices'		=> $dynamic_data,
			'multiple'		=> 0,
			'ui'			=> 1,
			'allow_null'	=> 1,
			'placeholder'   => __( "Select Dynamic Value",FEA_NS ),
		) );
		echo '</div>';
	...
	function columns_content( $column, $form_id ) {
		...
		if ( 'shortcode' == $column ) {
			...
			// Show shortcode
			echo sprintf( '<code>[frontend-admin form="%d"]</code>', $form_id );
			
			$icon_path = '<span class="dashicons dashicons-admin-page"></span>';
			//Save icon location
			echo sprintf( 
				'<button type="button" class="copy-shortcode" data-prefix="frontend-admin form" data-value="%1$s">%2$s %3$s</button>',
				$form_id,
				$icon_path,
				__( 'Copy Code', FEA_NS )
			);
