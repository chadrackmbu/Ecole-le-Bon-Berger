		...
		public function column_default( $item, $column_name ) {
			switch( $column_name ){
				...
				case 'title':
					...
					$page = wp_kses( $_REQUEST['page'], 'strip' );
					$title = sprintf( '<a href="?page=%s&action=%s&id=%s">%s</a>', $page, 'edit', $item['id'], $title );
					$actions = array(
						'edit' => sprintf('<a href="?page=%s&action=%s&id=%s">%s</a>', $page, 'edit', $item['id'], __( 'Review', FEA_NS ) ),
						'delete' => sprintf('<a href="?page=%s&action=%s&id=%s&nonce=%s">%s</a>', $page, 'delete', $item['id'], wp_create_nonce( 'frontend_admin_delete_submission' ), __( 'Delete', FEA_NS ) ),
					);
