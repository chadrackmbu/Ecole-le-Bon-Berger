(function($) {
	...
	$(document).ready(function() {
		...
		// Toggle dropdown
		$(document).on( 'focusin click', '.acf-field[data-dynamic_values] input, a.dynamic-value-options', function(e) {			
			e.stopPropagation();
		...
		$('body').on('change','select#form-admin_form_type',function(e){
	...
	(function ($, undefined) {
	  ...
	  (function ($, undefined) {
		...
		var ConditionalLogicFieldSetting = acf.FieldSetting.extend({
		  ...
		  events: {
			'change .acf-conditional-toggle': 'onChangeToggle',
		  ...
		  $toggle: function () {
			var toggle = this.$('.conditions-toggle');
	  ...
	  (function ($, undefined) {
		acf.FieldObject = acf.Model.extend({
		  ...
		  // events
		  events: {
			'click .acf-tab-button': 'onClickTab',
			...
			'input .field-label': 'onTypeLabel',
			'input .update-label': 'onTypeLabel',
		  ...
		  onClickEdit: function (e) {
			...
			this.$('.acf-tab-button[data-tab=general]').trigger('click');
		  ...
		  onClickTab: function( e, $el ){
			$el.closest('li').addClass('active').siblings('.active').removeClass('active');
			this.$('.acf-field-settings-main').hide();
			this.$('.acf-field-settings-main-'+$el.data('tab')).show();
		  },
		  ...
		  changeType: function (newType) {
			...
			this.$('.acf-field-settings-main').show();
			const ajaxData = {
			  action: 'fea/form/render_field_settings',
	  ...
	  (function ($, undefined) {
		...
		var fieldManager = new acf.Model({
		  ...
		  onClickAdd: function (e, $el) {
			...
			popup.on('click', '[data-type]', 'onConfirm');
			
		  },
		 
			add: function (args) {
					// vars
				var html = $('#tmpl-new-field').html();
				var $el = $(html);
				...
				newField.prop('label', '');
				if( args.type == 'step' ){
					newField.prop('label', 'Step');
					$newField.find('.field-type').val('form_step').trigger('change');
				}
				if( args.type == 'acf' ){
					newField.prop('label', 'ACF Fields');	
					$newField.find('.field-type').val('fields_select').trigger('change');
				}
				...
				this.renderFields(args.list); // action

				newField.$('.acf-tab-button[data-tab=general]').trigger('click');
