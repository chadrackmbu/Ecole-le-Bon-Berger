...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <div class="vx_wrap"> 
  <div><img alt="<?php esc_html_e("Constant Contact Feeds", 'gravity-forms-ccontact-crm') ?>" title="<?php esc_html_e("Constant Contact Feeds", 'gravity-forms-ccontact-crm') ?>" src="<?php echo $this->get_base_url()?>images/ccontact-crm-logo.png?ver=1" style="float:left; margin:0 7px 10px 0;" height="46" />
  <h2 style="margin-bottom: 12px"><?php esc_html_e("Constant Contact Log", 'gravity-forms-ccontact-crm'); ?></h2>
  <div class="clear"></div>
</div>
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php esc_html_e('Search','gravity-forms-ccontact-crm') ?>" value="<?php echo esc_attr($this->post('search')) ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php esc_html_e('All Objects','gravity-forms-ccontact-crm') ?></option>
  <?php    
  ...
  <option value=""><?php esc_html_e('All Status','gravity-forms-ccontact-crm') ?></option>
  <?php
  ...
  <option value=""><?php esc_html_e('All Times','gravity-forms-ccontact-crm') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php esc_html_e('From Date','gravity-forms-ccontact-crm') ?>From Date" value="<?php if(isset($_REQUEST['start_date'])){echo esc_attr($this->post('start_date'));}?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php if(isset($_REQUEST['end_date'])){echo esc_attr($this->post('end_date'));}?>" placeholder="<?php esc_html_e('To Date','gravity-forms-ccontact-crm') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" title="<?php esc_html_e('Search','gravity-forms-ccontact-crm') ?>" name="search" class="button-secondary button crm_input_inline"><i class="fa fa-search"></i> <?php esc_html_e('Search','gravity-forms-ccontact-crm') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php esc_html_e('Apply','gravity-forms-ccontact-crm') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php esc_html_e('Apply','gravity-forms-ccontact-crm') ?></button>
  ...
  ?>
  <button type="button" name="tab_action" title="<?php esc_html_e('Export as CSV','gravity-forms-ccontact-crm') ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php esc_html_e('Export as CSV','gravity-forms-ccontact-crm') ?></button> 
  <?php
  }
        if($this->post('log_id') !="" ){
  if(isset($data['feeds'][0]['entry_id']) && $data['feeds'][0]['entry_id']!=""){
     $entry_id=$data['feeds'][0]['entry_id'];
      ?>
  <a href="<?php echo esc_url($log_link.'&entry_id='.$entry_id);?>" title="<?php echo sprintf(__('View Entry# %s Logs','gravity-forms-ccontact-crm'),$entry_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf(__('View Entry# %s Logs','gravity-forms-ccontact-crm'),$entry_id); ?></a><?php
  }}
 if($this->post('entry_id') !="" || $this->post('log_id') !=""){
          ?><a href="<?php echo esc_url($log_link);?>" title="<?php esc_html_e('View All Logs','gravity-forms-ccontact-crm') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php esc_html_e('View All Logs','gravity-forms-ccontact-crm') ?></a>        
          <?php
      }
...
if($items>0){
  ?>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','gravity-forms-ccontact-crm') ?> <span id="paging_range_min_header"><?php echo esc_html($data['min']) ?></span> - <span id="paging_range_max_header"><?php echo esc_html($data['max']) ?></span> of <span id="paging_total_header"><?php echo esc_html($data['items']) ?></span></span><?php echo wp_kses_post($data['links']); ?></div>
 <?php
}
        ?>       
  </div>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php esc_html_e("Constant Contact ID", 'gravity-forms-ccontact-crm') ?>
  <i class="fa fa-caret-<?php echo esc_attr($crm_order) ?> vx_sort_icon <?php echo esc_attr($crm_class) ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="entry_id"><?php esc_html_e("Entry ID", 'gravity-forms-ccontact-crm') ?>
  <i class="fa fa-caret-<?php echo esc_attr($entry_order) ?> vx_sort_icon <?php echo esc_attr($entry_class) ?>"></i>                      
  </th>
  <th scope="col" class="manage-column"  data-name="object"><?php esc_html_e("Description", 'gravity-forms-ccontact-crm') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php esc_html_e("Time", 'gravity-forms-ccontact-crm') ?>
  <i class="fa fa-caret-<?php echo esc_attr($time_order) ?> vx_sort_icon <?php echo esc_attr($time_class) ?>"></i>
  </th>
  <th style="width: 40px"><?php esc_html_e('Detail','gravity-forms-ccontact-crm') ?></th>
  </tr>
  </thead>
  ...
  <th><?php esc_html_e('Detail','gravity-forms-ccontact-crm') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo esc_attr($row['id']) ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo $base_url ?>images/<?php echo esc_attr($row["status_img"]) ?>.png" alt="<?php echo $row["status"] ? esc_html__("Active", 'gravity-forms-ccontact-crm') : esc_html__("Inactive", 'gravity-forms-ccontact-crm');?>" title="<?php echo esc_attr($row['title'])?>" class="crm_status_img" /></td>
  <td class="column-name" style="width:40%"><p><?php echo wp_kses_post($row['a_link']) ?></p></td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php esc_html_e('Expand Details','gravity-forms-ccontact-crm') ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php esc_html_e("No Record(s) Found", 'gravity-forms-ccontact-crm'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php esc_html_e('Clear Constant Contact Log','gravity-forms-ccontact-crm') ?>" href="<?php echo wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce'); ?>"><?php esc_html_e('Clear Constant Contact Log','gravity-forms-ccontact-crm') ?></a>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','gravity-forms-ccontact-crm') ?> <span id="paging_range_min_header"><?php echo esc_html($data['min']) ?></span> - <span id="paging_range_max_header"><?php echo esc_html($data['max']) ?></span> of <span id="paging_total_header"><?php echo esc_html($data['items']) ?></span></span><?php echo wp_kses_post($data['links']); ?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php esc_html_e('Please Select Action','gravity-forms-ccontact-crm') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php esc_html_e('Please select at least one entry','gravity-forms-ccontact-crm') ?>');
    return false;
}
var action=sel.val();
if( $.inArray(action,["send_to_crm_bulk_force","send_to_crm_bulk"]) !=-1 && $(".crm_input_check:checked").length>4){
 if(!confirm('<?php esc_html_e('Exporting more than 4 entries may take too long.\\n Are you sure you want to continue?','gravity-forms-ccontact-crm') ?>')){
  e.preventDefault();    
 }   
}
  })
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php esc_html_e('Constant Contact Logs will be deleted permanently. Do you want to continue?','gravity-forms-ccontact-crm') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(td.is(":visible"))
    {
       ...
       if(is_main){
    icon.attr('title','<?php esc_html_e('Expand Detail','gravity-forms-ccontact-crm'); ?>');    
    }        
     return;   
    }else{
    next_tr.show('fast'); 
    ...
    if(is_main){
    icon.attr('title','<?php esc_html_e('Collapse Detail','gravity-forms-ccontact-crm'); ?>');    
    }    
    }
