...
.vxcf_options_row .howto{
  ...
  <div class="vx_wrap"> <img alt="<?php esc_html_e("Constant Contact Feeds", 'gravity-forms-ccontact-crm') ?>" title="<?php esc_html_e("Constant Contact Feeds", 'gravity-forms-ccontact-crm') ?>" src="<?php echo $this->get_base_url()?>images/<?php echo $this->crm_name ?>-crm-logo.png?ver=1" style="float:left; margin:0px 7px 10px 0;" height="46" />
  <h2 style="margin-bottom: 12px"><?php esc_html_e("Constant Contact Feeds", 'gravity-forms-ccontact-crm'); ?>
  <?php
      if($fid != $config['id']){
  ?>
  <a class="add-new-h2" href="<?php echo esc_url($new_feed_link)?>"><?php esc_html_e("Add New", 'gravity-forms-ccontact-crm') ?></a>
  <?php
      }
      if(!empty($fid)){
  ?>
  <a class="add-new-h2" href="<?php echo esc_url($feeds_link)?>"><?php esc_html_e("Back to Feeds", 'gravity-forms-ccontact-crm') ?></a>
  <?php
      }
  ?>
  </h2>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(__("We are unable to login to Constant Contact with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'gravity-forms-ccontact-crm'), "<a href='".$this->link_to_settings()."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  <div class="vx_wrapper">
  <div class="vx_heading"><?php echo sprintf(__('Edit Constant Contact Feed # %d','gravity-forms-ccontact-crm') ,$fid) ?></div>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('1. Enter Feed name',  'gravity-forms-ccontact-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php esc_html_e("Feed Name", 'gravity-forms-ccontact-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('2. Select Constant Contact Account.', 'gravity-forms-ccontact-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','gravity-forms-ccontact-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php esc_html_e("Constant Contact Account", 'gravity-forms-ccontact-crm'); ?>
  ...
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php esc_html_e('Loading, Please Wait...','gravity-forms-ccontact-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','gravity-forms-ccontact-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php esc_html_e('Are you sure to remove ?','gravity-forms-ccontact-crm') ?>')){
     return;   
    }
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','gravity-forms-ccontact-crm') ?>');
     return;   
    }  
...
if(field.label){
 temp.find('.crm_text_label').text(field.label);
  ...
  function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','gravity-forms-ccontact-crm') ?>'});
} 
...
vx_select();
  ...
  vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','gravity-forms-ccontact-crm') ?>'});
