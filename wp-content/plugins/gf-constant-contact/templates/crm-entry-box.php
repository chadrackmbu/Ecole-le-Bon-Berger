...
<div class="postbox">
<h3 style="border-bottom: 1px solid #ddd;"><?php esc_html_e("Constant Contact", 'gravity-forms-ccontact-crm') ?></h3>
<div class="inside">
...
<?php
  ...
  if( is_array($log) && count($log)>0){
      ...
      $log=$this->verify_log($log,$objects); 
...
echo '<p style="background: '.esc_attr($bg).'; padding: 10px; color: #fff " class="vx_msg_div"><i class="fa '.esc_attr($icon).'"></i> '.wp_kses_post($msg).'</p>';
  ...
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo esc_attr($this->id) ?>_send" value="yes" title="<?php esc_html_e("Send to Constant Contact",'gravity-forms-ccontact-crm')?>"><?php esc_html_e("Send to Constant Contact",'gravity-forms-ccontact-crm')?></button>
  <?php
      if($comments ){
  ?>
    <a class="button button-secondary" title="<?php esc_html_e('Go to Logs','gravity-forms-ccontact-crm') ?>" href="<?php echo esc_url($log_url) ?>"><?php esc_html_e('Logs','gravity-forms-ccontact-crm') ?></a>
  <?php
      }
  ?>
  </p>
