...
if( !class_exists( 'vxg_ccontact_form' ) ) {
class vxg_ccontact_form extends vxg_ccontact{
...
$feed=get_option($this->id.'_lists',array()); 
  ...
  /**
  * Replace the Gravity Forms form choices with remotely-pulled Constant Contact picklist options.
  ...
  */
  ...
  foreach($form['fields'] as $field) {// var_dump($this->post('crm_map_type',$field));
  // If the field has mapping enabled, and the object and field are defined, replace it
  if( $this->post('crm_map_enabled',$field) ==$this->id &&
  $this->post('crm_object',$field)!="" &&
  $this->post('crm_option',$field)!="" &&
  $this->post('crm_map_type',$field) == "live"
  ){
  $crm_field=$this->post('crm_option',$field); //crm field name
  $crm_object=$this->post('crm_object',$field); 
  $crm_account=$this->post('crm_account',$field); 
  if($crm_field!=""){ 
...
if(isset($objects[$object])){
  ...
  */
  ...
  $choices[] = array(
  'text' => $value['value'],
  'value' => isset($value['name']) ? $value['name'] : $value['value'],
 ...
 public function fields_html($id,$object,$refresh=false){
  ...
  $fields = $this->get_field_options($id,$object,$refresh); 
  if(!is_array($fields)){
      $msg= empty($fields) ? esc_html__('Fields Not Found','gravity-forms-ccontact-crm') : $fields;
  $str = '<div class="field_group_'.$this->id.'"><strong>'.__('Error:', 'gravity-forms-ccontact-crm').'</strong> '.wp_kses_post($msg).'</div>';
  ...
  */
  ...
  if(empty($value['value'])) { continue; }
        $default_html = !empty($value['default']) ?  '<strong class="default"> '.esc_html__('(Default)', 'gravity-forms-ccontact-crm').'</strong>' : '';
        $default = !empty($value['default']) ?  '1' : '0';
        $name=isset($value['name']) ? $value['name'] : $value['value'];
  $str .= '<li style="margin:0; padding:0;" data-default="'.$default.'" data-value="'.htmlentities($name).'" data-label="'.htmlentities($value['value']).'">'.htmlentities($value['value']).' '.$default_html.'</li>';
  }
  $str .= '</ul>';
  return $str;
  }
  ...
  */
  ...
  if($str !=''){
  $str="
  <div id='crm_field_group_form'>
  <table cellpadding='0' cellspacing='0' class='form-table'>
  <thead class='screen-reader-text'>
  <tr>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Pickist Field", 'gravity-forms-ccontact-crm') . "</th>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Form Fields", 'gravity-forms-ccontact-crm') . "</th>
  </tr>
  </thead>
  <tbody>".$str."
  </tbody>
  ...
  $str='<div><b>'.__('No Options Found','gravity-forms-ccontact-crm').'</b></div>';    
  }
  /// $str = str_replace(array("\n", "\t", "\r"), '', str_replace("'", "\'", $str));
...
//  toggle_choices(check_box);  
  ...
  */
  ...
  $tooltips['vx_map_live_'.$this->id] = sprintf(__('%sUpdate from Constant Contact %s If you update a picklist in Constant Contact, the modifications will be added to your form without having to edit the field Choices in Gravity Forms. You will not be able to edit the Choices in Gravity Forms, you can only update them in Constant Contact. The order of Choices as well as the default values are determined by the Constant Contact picklist field settings.', 'gravity-forms-ccontact-crm'), '<h6>','</h6>');
  $tooltips['vx_map_once_'.$this->id] = sprintf(__('%sUpdate from Constant Contact %s Field Choices will not be updated live from Constant Contact and are editable in Gravity Forms. If you make an edit in Constant Contact, it will not be updated in your form.', 'gravity-forms-ccontact-crm'), '<h6>','</h6>');
  return $tooltips;
  }
  /**
  ...
  */
  ...
  <input type="checkbox" autocomplete="off" class="<?php echo esc_attr($this->id) ?>_map_enabled crm_map_enabled" id="vx_check_<?php echo esc_attr($this->id) ?>" name="crm_map_enabled" value="<?php echo esc_attr($this->id) ?>" /> <?php esc_html_e("Enable Constant Contact Field Mapping?", 'gravity-forms-ccontact-crm'); ?>
  </label>
  
  <div id="<?php echo esc_attr($this->id) ?>_map_ui" style="display: none;">
  
  <label for="<?php echo esc_attr($this->id) ?>_map_type_live">
  <input type="radio" class="<?php echo esc_attr($this->id) ?>_map_type crm_map_type" id="<?php echo esc_attr($this->id) ?>_map_type_live" name="<?php echo esc_attr($this->id) ?>_map_type" value="live" /> <?php esc_html_e("Live Remote Field Mapping ", 'gravity-forms-ccontact-crm'); gform_tooltip("vx_map_live_".$this->id); ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will be synced from Constant Contact picklist values.", 'gravity-forms-ccontact-crm'); ?></span>
  </label>
  <label for="<?php echo esc_attr($this->id) ?>_map_type_once">
  <input type="radio" class="<?php echo esc_attr($this->id) ?>_map_type crm_map_type" id="<?php echo esc_attr($this->id) ?>_map_type_once" name="<?php echo esc_attr($this->id) ?>_map_type" value="once" /> <?php esc_html_e("Only Populate Choices ", 'gravity-forms-ccontact-crm'); gform_tooltip("vx_map_once_".$this->id) ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will not be updated live and are editable.", 'gravity-forms-ccontact-crm'); ?></span>
  </label>
    <label for="crm_accounts_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Account", 'gravity-forms-ccontact-crm'); ?></label>
  ...
  <option value=""><?php esc_html_e("Select a Constant Contact Account", 'gravity-forms-ccontact-crm'); ?></option>
  <?php
 ...
 public function object_area($id,$object){
     ...
     ?>
       <label for="crm_object_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Object", 'gravity-forms-ccontact-crm'); ?></label>
  <?php
  if(!is_array($lists)) {
  echo esc_html__("Could not load Constant Contact objects.", 'gravity-forms-ccontact-crm');
  } else { ?>
  ...
  <option value=""><?php esc_html_e("Select a Constant Contact Object", 'gravity-forms-ccontact-crm'); ?></option>
  <?php
