...
<div class="ulangan-card exam-result skwp-content-inner skwp-clearfix">
	...
	<?php 
		...
		if ($number > 0):
		for ($i = 0; $i <= $number; $i++):
		$data = $wpdb->get_results( "SELECT correct_answer, marks FROM {$wpdb->prefix}sakolawp_questions WHERE question_id = '$ids[$i]'", ARRAY_A );
		...
		<div class="ulangan-info skwp-clearfix">
			<div class="skwp-row">
				...
				<div class="info-right info-det skwp-column skwp-column-2">
					<ul>
						<li>
							<span class="title"><?php esc_html_e('Mark', 'sakolawp'); ?></span><span class="isi"><button href="" class="btn btn-primary btn-sm btn-rounded"><?php echo intval(round((($cor/$row['questions']) * 100)));?></button></span></li>
					...
					<tr>
						...
						<td>
							<a class="btn btn-rounded btn-sm btn-success skwp-btn">
								<?php if($row3['optiona'] == $row3['correct_answer']) { ?><?php esc_html_e('A', 'sakolawp'); ?><?php } ?>
								<?php if($row3['optionb'] == $row3['correct_answer']) { ?><?php esc_html_e('B', 'sakolawp'); ?><?php } ?>
								<?php if($row3['optionc'] == $row3['correct_answer']) { ?><?php esc_html_e('C', 'sakolawp'); ?><?php } ?>
								<?php if($row3['optiond'] == $row3['correct_answer']) { ?><?php esc_html_e('D', 'sakolawp'); ?><?php } ?>
							</a>
						</td>
						<td>
						<?php if($row3['correct_answer'] == $ans[$i]):?>
							...
							<a class="btn nc btn-rounded btn-sm btn-success skwp-btn">
								<?php if($row3['optiona'] == $ans[$i]) { ?><?php esc_html_e('A', 'sakolawp'); ?><?php } ?>
								<?php if($row3['optionb'] == $ans[$i]) { ?><?php esc_html_e('B', 'sakolawp'); ?><?php } ?>
								<?php if($row3['optionc'] == $ans[$i]) { ?><?php esc_html_e('C', 'sakolawp'); ?><?php } ?>
								<?php if($row3['optiond'] == $ans[$i]) { ?><?php esc_html_e('D', 'sakolawp'); ?><?php } ?>
							</a>
						<?php endif;?>
						<?php if($ans[$i] == ''):?>
