...
if (!class_exists('Wpmet\Libs\Rating')) {
    ...
    class Rating
    {
        ...
        {
            if( empty( $_POST['nonce'] ) || ( isset( $_POST['nonce'] ) && ! wp_verify_nonce( sanitize_text_field(  $_POST['nonce']  ), 'wpmet_rating' ) ) ){
				return false;
			}
                    ...
                    $( '#".$this->text_domain."_btn_already_did' ).on( 'click', function() {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                plugin_name : '<?php echo esc_html( $this->text_domain);?>',
                                nonce : '<?php echo esc_js( wp_create_nonce( 'wpmet_rating' ) );?>'
                            },
                            success:function(response){
                                $('#<?php echo esc_html( $this->text_domain);?>-_plugin_rating_msg_used_in_day').remove();
                            }
                        });
