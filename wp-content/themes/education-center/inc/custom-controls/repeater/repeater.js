/*jshint -W065 */
var educationcenterproRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['education-center-repeater'] = wp.customize.Control.extend({
	ready: function(){
		...
		// The current value set in Control Class (set in Education_Center_Control_Repeater::to_json() function)
	...
	addRow: function( data ){
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Education_Center_Control_Repeater::render_content() ).
		...
		if ( template ) {
			...
			newRow = new educationcenterproRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'education_center_get_fontawesome_ajax',
					education_center_customize_nonce: education_center_customize.nonce
				},
				beforeSend: function(){
